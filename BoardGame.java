...
public class BoardGame {
    ...
    public void processClick(int x, int y, JPanel screen) {
        if (phase == passToPlayer) {
            phase = chooseRedCard;
        } else if (phase == chooseRedCard) {
            int selectedCard = selectedHandCard(x, y);
            if (selectedCard >= 0) {
            ...
            int winner = selectedSubmission(x, y);
            if (winner >= 0) {
                awardGreenCard(winner, screen);
            }
        }
    }
    ...
    private void awardGreenCard(int submissionIndex, JPanel screen) {
        ...
        if (greenCardsWon[winner] >= greenCardsToWin) {
            phase = gameOver;
            Window window = SwingUtilities.getWindowAncestor(screen);
            if (window != null) {
                window.dispose();
            }
